...
public class PatientGraph {
	...
	public Intent getIntent(Context context) {
		...
		XYMultipleSeriesDataset dataset = getDataset(context, patientId, dateFormat);
		XYMultipleSeriesRenderer mRenderer = getRenderer();
		
		
		Intent intent = ChartFactory.getTimeChartIntent(context, dataset, mRenderer, "dd/MM/yy hh:mm");
		return intent;
	}
	...
	public GraphicalView getView(Context context, int patientId) {
		...
		File dir = new File(".");
		try {
			System.out.println("Curr Dir " + dir.getAbsolutePath() );
			System.out.println("Curr Dir " + dir.getPath() );
			System.out.println("Curr Dir " + dir.getName() );
		}
		catch (Exception e) {
		...
		*/
		...
		return ChartFactory.getTimeChartView(context, dataset, mRenderer, "dd/MM/yy hh:mm");
	...
	private void checkParameter(XYMultipleSeriesDataset dataset, XYMultipleSeriesRenderer renderer) {
	...
	private XYMultipleSeriesDataset getDatasetEmpty(Context context, int patientId, String dateFormat) {
	...
	private XYMultipleSeriesDataset getDataset(Context context, int patientId, String dateFormat) {
		RespiratoryDataSource rds = new RespiratoryDataSource(context); 
		List<Respiratory> resList = new ArrayList<Respiratory>();
		...
		rds.open();
		resList = rds.getRespiratoryByPatientIdSortByTime(patientId);
		rds.close();
